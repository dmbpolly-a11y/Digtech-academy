...
import { TutorRow } from '@/app/principal/tutors/TutorRow';
...
export default async function AdminTutorsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-8">
      ...
      <p className="mt-1 text-sm text-ink/60">Full tutor management — same controls available to Principals.</p>
      ...
      <div className="mt-6 space-y-3">
        ...
        {(!tutors || tutors.length === 0) && (
          <p className="rounded-xl2 border border-dashed border-brand-light p-8 text-center text-sm text-ink/50">No tutors yet.</p>
        )}
      </div>
    </div>
  );
